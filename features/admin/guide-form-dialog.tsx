...
import { guideArticleSchema, type GuideArticleInput } from "@/lib/validations/admin";
import { createGuideArticleAction, updateGuideArticleAction } from "@/actions/admin";
...
export function GuideFormDialog({ article, trigger }: GuideFormDialogProps) {
  ...
  const defaultValues: GuideArticleInput = {
    title: article?.title ?? "",
    slug: article?.slug ?? "",
  ...
  async function onSubmit(values: GuideArticleInput) {
    ...
    const result = isEdit
      ? await updateGuideArticleAction({ id: article!.id, ...values })
      : await createGuideArticleAction(values);
